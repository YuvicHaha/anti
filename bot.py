...
intents.message_content = True  # Important: enable message content intent
...
async def on_message(message):
    ...
    if message.content.startswith(".antirep"):
        try:
            ...
            if len(args) != 3:
                await message.channel.send("⚠️ Usage: `.replace <oldid> <newid>`")
                return
            oldid = int(args[1])
            newid = int(args[2])
    ...
    await bot.process_commands(message)  # Allow @bot.command to still work
@bot.command()
        ...
        for line in lines:
            if f"--- {discord_id}" in line:
                ...
                if res.status_code == 200:
                    ...
                    embed = discord.Embed(
                        title="Your Anti Stand is Linked To",
